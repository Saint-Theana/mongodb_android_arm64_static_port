...
krb5_keyblock master_keyblock_fuck;
    ...
    char *argv[];
{
    ...
    master_keyblock_fuck.enctype = global_params.enctype;
    if ((master_keyblock_fuck.enctype != ENCTYPE_UNKNOWN) &&
        (!krb5_c_valid_enctype(master_keyblock_fuck.enctype))) {
        com_err(progname, KRB5_PROG_KEYTYPE_NOSUPP,
                "while setting up enctype %d", master_keyblock_fuck.enctype);
    }
...
static int open_db_and_mkey()
{
    ...
    if (mkey_password) {
        ...
        if (master_keyblock_fuck.enctype == ENCTYPE_UNKNOWN)
            master_keyblock_fuck.enctype = DEFAULT_KDC_ENCTYPE;
        if (!krb5_c_valid_enctype(master_keyblock_fuck.enctype))
            com_err(progname, KRB5_PROG_KEYTYPE_NOSUPP,
                    "while setting up enctype %d",
                    master_keyblock_fuck.enctype);

        retval = krb5_c_string_to_key(util_context, master_keyblock_fuck.enctype,
                                      &pwd, &scratch, &master_keyblock_fuck);
        if (retval) {
            com_err(progname, retval,
    ...
    seed.length = master_keyblock_fuck.length;
    seed.data = (char *) master_keyblock_fuck.contents;

    if ((retval = krb5_c_random_seed(util_context, &seed))) {
        com_err(progname, retval, _("while seeding random number generator"));
        exit_status++;
        memset(master_keyblock_fuck.contents, 0, master_keyblock_fuck.length);
        krb5_free_keyblock_contents(util_context, &master_keyblock_fuck);
        return(1);
    }
...
quit()
{
    ...
    zapfree(master_keyblock_fuck.contents, master_keyblock_fuck.length);
