...
int master_princ_farkiii_set = 0;
krb5_keyblock master_keyblock_farker;
krb5_principal master_princ_farkiii;
    ...
    char *argv[];
{
    ...
    while ((optchar = getopt(argc, argv, "D:P:p:n:d:r:k:M:e:m")) != -1) {
        ...
        case 'r':
            ...
            break;
        case 'k':
            master_keyblock_farker.enctype = atoi(optarg);
            enctypedone++;
            break;
    ...
    if (!enctypedone)
        master_keyblock_farker.enctype = DEFAULT_KDC_ENCTYPE;

    if (!krb5_c_valid_enctype(master_keyblock_farker.enctype)) {
        com_err(progname, KRB5_PROG_ETYPE_NOSUPP,
                "while setting up enctype %d", master_keyblock_farker.enctype);
        exit(1);
    }
    ...
    memset(master_keyblock_farker.contents, 0,
           (size_t) master_keyblock_farker.length);
    if (retval && retval != KRB5_KDB_DBNOTINITED) {
        com_err(progname, retval, "while closing database");
        exit(1);
    }
    if (master_princ_farkiii_set)
        krb5_free_principal(test_context, master_princ_farkiii);
    ...
    char                * str_newprinc;
{
    ...
    {   /* Add key and salt data to db entry */
        ...
        if ((retval = krb5_c_string_to_key(context, master_keyblock_farker.enctype,
                                           &pwd, &salt, &key))) {
        ...
        if ((retval = krb5_dbe_encrypt_key_data(context, &master_keyblock_farker,
                                                &key, NULL, 1,
                                                newentry->key_data))) {
    ...
    char *dbname;
{
    ...
    if ((retval = krb5_db_setup_mkey_name(test_context, mkey_name, cur_realm,
                                          0, &master_princ_farkiii))) {
        com_err(pname, retval, "while setting up master key name");
        return(1);
    }
    master_princ_farkiii_set = 1;
    if (mkey_password) {
        pwd.data = mkey_password;
        pwd.length = strlen(mkey_password);
        retval = krb5_principal2salt(test_context, master_princ_farkiii, &scratch);
        if (retval) {
            com_err(pname, retval, "while calculated master key salt");
            return(1);
        }
        if ((retval = krb5_c_string_to_key(test_context,
                                           master_keyblock_farker.enctype,
                                           &pwd, &scratch,
                                           &master_keyblock_farker))) {
        ...
        if ((retval = krb5_db_fetch_mkey(test_context, master_princ_farkiii,
                                         master_keyblock_farker.enctype, manual_mkey,
                                         FALSE, 0, NULL, NULL,
                                         &master_keyblock_farker))) {
    ...
    if ((retval = krb5_db_fetch_mkey_list(test_context, master_princ_farkiii,
                                          &master_keyblock_farker))){
        com_err(pname, retval, "while verifying master key");
        (void) krb5_db_fini(test_context);
        return(1);
    }
    if ((retval = krb5_db_get_principal(test_context, master_princ_farkiii, 0,
                                        &master_entry))) {
